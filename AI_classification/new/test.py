...
def build_train_cmd(train_script: Path, input_dir: Path, output_dir: Path, tflite_model: Path, tour_id: int, skip_pytorch: bool):
    cmd = [
        sys.executable,
        train_script,
        "--input-dir", input_dir,
...
def build_infer_cmd(inference_script: Path, image_path: Path, mode: str, checkpoint: Optional[Path], index_json: Optional[Path], tflite_model: Optional[Path], skip_geometry: bool):
    """
    mode:
